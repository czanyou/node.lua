...
local len
local _stateRoundUpToNextPowerOf2
...
function ReadableState:initialize(options)
    options = options or { }

    -- the point at which it stops calling _read() to fill the buffer
    -- Note: 0 is a valid value, means "don't call _read preemptively ever"
    local defaultHighWaterMark = 16
    if not options.objectMode then
    defaultHighWaterMark = 16 * 1024
    end

    self.highWaterMark = options.highWaterMark or defaultHighWaterMark -- 水位标记, 用来触发 readable 等事件的临界点
    ...
    -- self.readableEnded      = false   -- 指出当前流是否已结束, 表示底层数据源已读完或关闭不会再提供更多数据了
...
function ReadableState:_stateNeedMoreData()
    return self.needReadable or (self.length < self.highWaterMark) or (self.length == 0)
end

-- 写入指定的数据包到缓存区中
function ReadableState:_stateWriteToBuffer(chunk, addToFront)
...
-- 检查 chunk 类型是否合法
function ReadableState:_stateCheckChunk(chunk)
    local err = nil
    if chunk and (type(chunk) ~= 'string') and (not self.objectMode) then
...
function ReadableState:_stateReadFromBuffer(n)
    n = self:_stateHowMuchToRead(n)
    ...
    local objectMode = not not self.objectMode
    local ret
...
function ReadableState:_stateHowMuchToRead(n, readableEnded)
    if readableEnded and (self.length == 0) then
        return 0
    end
    ...
    if (n ~= n) or (not n) then
        if self.flowing then
            -- 流模式下只返回一个数据包
            local buffer = self.buffer[1]
            if (buffer) then
                return #buffer
            end
    ...
    -- amounts.
    --]]
    if n > self.highWaterMark then
        self.highWaterMark = _stateRoundUpToNextPowerOf2(n)
    end
...
function Readable:initialize(options)
    ...
    self.readableHighWaterMark = nil
...
function Readable:push(chunk)
    return self:_readablePushChunk(chunk, false)
end
...
function Readable:read(n)
    ...
    if (n == 0) and state.needReadable then
        if self.readableEnded then
            if (state.length == 0) then
                self:_readableEmitEnd()
            else
                self:_readableEmitReadable()
            end
            return nil
    ...
    if self.readableEnded and (state.length == 0) then
        -- 流已经标记为结束 & 缓存区已空
        self:_readableEmitEnd()
        return nil
    end
    ...
    self:_readableRead(n)

    local ret = self:_readableReadFromBuffer(n)
    if ret ~= nil then
...
function Readable:pipe(dest, pipeOptions)
    local src = self
    local state = self._readableState
    ...
    local doEnd = (not pipeOptions or pipeOptions._end ~= false)
    ...
    -- too slow.
    local function _readablePipeOnDrain(src)
        return function()
    ...
    elseif (event == 'readable') then
        if (not state.endEmitted) and (not state.readableListening) then
            ...
            elseif state.length then
                self:_readableEmitReadable(state)
            end
        end
    end
...
function Readable:unshift(chunk)
    return self:_readablePushChunk(chunk, true)
end

-- 调用这个方法让指定的流进入流模式
function Readable:_readableResume(state)
    ...
    process.nextTick(function()
        self:_readableResumeNextTick(state)
    end)
end

function Readable:_readableResumeNextTick(state)
    if (not state.reading) then
        self:read(0)  -- 触发数据读取
    end
...
-- 从底层读取更多的数据
function Readable:_readableRead(n)
    local state = self._readableState
   ...
   --[[
    ...
    -- synthetic self cases, such as passthrough streams, _read
    ...
    elseif (n and n > 0) then
        n = state:_stateHowMuchToRead(n, self.readableEnded)
...
function Readable:_readableReadFromBuffer(n)
    local state = self._readableState

    local ret = state:_stateReadFromBuffer(n)
    if not ret then
    ...
    if (state.length == 0) and (not self.readableEnded) then
        state.needReadable = true
    end

    --
    -- If we tried to read() past the EOF, then emit end on the next tick.
    --if (nOrig ~= n) and self.readableEnded and (state.length == 0) then
    --    _readableEmitEnd()
    --end
...
-- 结束这个流, 并发出 'end' 的事件
function Readable:_readableEmitEnd()
    local state = self._readableState
...
function Readable:_readableEmitReadable()
    local state = self._readableState
...
function Readable:_readableFlow()
    local state = self._readableState
    while state.flowing and (self:read() ~= nil)  do
        --...
    end
end
...
-- 返回值指示是否可以继续 push 数据
function Readable:_readablePushChunk(chunk, addToFront)
    local state = self._readableState
    local err = state:_stateCheckChunk(chunk)
    if err then
        -- 传入了无效的参数
        self:emit('error', err)
    ...
    elseif chunk == nil then
        ...
        state.reading = false
        if not self.readableEnded then
            self.readableEnded = true
    ...
    elseif state.objectMode or chunk and len(chunk) > 0 then
        if self.readableEnded and not addToFront then
            local err = Error:new('self.push() after EOF')
            self:emit('error', err)
            ...
            if state.flowing and (state.length == 0) and (not state.sync) then
                ...
                state:_stateWriteToBuffer(chunk, addToFront)

                if state.needReadable then
                    self:_readableEmitReadable()
                end

                self:_readableMaybeReadMore(state)
            end
        end
    ...
    return state:_stateNeedMoreData()
end
...
function Readable:_readableMaybeReadMore(state)
    ...
    process.nextTick(function()
        self:_readableMaybeReadMoreNextTick(state)
    end)
end

function Readable:_readableMaybeReadMoreNextTick(state)
    local lastLength = state.length
    while
            not state.reading
        and not state.flowing
        and not self.readableEnded
