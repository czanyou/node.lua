...
local utf8_codes = nil
local gb2312_codes = nil

exports.initCodeTable = function()
    local data = gb2312data
    ...
    for i, line in ipairs(lines) do
        ...
        for pos, code in utf8.codes(line) do
            ...
            b = b + 1
            local a = 0xB0 + i - 1;
            utf8_codes[code] = string.char(a, b)
            gb2312_codes[(a<< 8) + b] = code
        end
    end
...
exports.encode = function(data, encoding)
    if (not utf8_codes) then
        exports.initCodeTable()
    end

    local result = {}
    for _, charCode in utf8.codes(data) do
        table.insert(result, utf8_codes[charCode] or string.char(charCode))
    end
...
exports.decode = function(data, encoding)
    ...
    while (index <= total) do
        local charCode = data:byte(index)
        if (charCode >= 0xA0) then
            index = index + 1
            charCode = (charCode << 8) + (data:byte(index) or 0)
            charCode = gb2312_codes[charCode]
            table.insert(result, utf8.char(charCode or 0))
