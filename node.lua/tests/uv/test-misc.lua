...
test("uv.os_gethostname", function (print, p, expect, uv)
    local path = assert(uv.os_gethostname())
    p('os_gethostname', path)
end)

test("uv.os_getenv", function (print, p, expect, uv)
    local path = assert(uv.os_getenv('PATH'))
    p('os_getenv', path)
end)

test("uv.os_setenv", function (print, p, expect, uv)
    uv.os_setenv('TEST', 'TEST')
    local path = assert(uv.os_getenv('TEST'))
    p('os_setenv', path)
end)

test("uv.os_unsetenv", function (print, p, expect, uv)
    uv.os_unsetenv('TEST')
