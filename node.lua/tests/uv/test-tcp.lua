...
if (isWindows) then sockname = "\\\\?\\pipe\\uv-test"; end
...
return tap(function (test)
    ...
    test("basic pipe server and client", function (print, p, expect, uv)
        ...
        local req = uv.pipe_connect(client, sockname, expect(function (err)
            --p("client on connect", client, err)
            assert(not err, err)
            uv.shutdown(client, expect(function (err)
               -- p("client on shutdown", client, err)
  ...
  test("pipe echo server and client", function (print, p, expect, uv)
  ...
  test("uv.tcp_bind invalid ip address", function (print, p, expect, uv)
