(function($) {
    $(document).ready(function() {
        var __NO_LOGIN__ = true

        var domain = document.domain || window.location.host
        var is_small_window = $(document).width() < 625 ? true : false
        var settings = {'url': 'webconsole.lua', 'prompt_path_length': 32,
                        'domain': domain, 'is_small_window': is_small_window};
        var environment = {'user': 'admin', 'hostname': '', 'path': ''};
        ...
        function make_prompt() {
            ...
            var user = (environment.user || 'user')
            var hostname = (environment.hostname || settings.domain || 'web-console')
            var path = (path || '~')
            return '[[b;#d33682;]' + user + ']' + '@[[b;#6c71c4;]' + hostname + '] ' + path + '$ ';
        }
        ...
        function interpreter(command, terminal) {
            ...
            if (method) {
                service_authenticated(terminal, method, parameters, function(result, data) {
        ...
        // Terminal
        var terminal = $('body').terminal(interpreter, options);
