...
local function data_stream_get_group_type(mode)
	if (mode == 'day') then
		return "strftime('%Y-%m-%d-%H', timestamp, 'unixepoch', 'localtime')"
...
local function data_stream_find(db, dataType, mode, offset, count)
	...
	count = count or 20
	local interval = (endTime - startTime) / count
	console.log('interval', interval, count)

	local groupBy = " GROUP BY " .. data_stream_get_group_type(mode)
	local orderBy = " ORDER BY timestamp"
	...
	--console.log(dataId, startTime, endTime)
...
local function data_stream_get_stat_group_type(mode)
	if (mode == 'day') then
...
local function data_stream_stat(db, dataType, mode, offset, count)
	mode   = mode or exports.DAY
	offset = offset or 0
	...
	local groupBy = " GROUP BY " .. data_stream_get_stat_group_type(mode)
	local orderBy = " ORDER BY timestamp"
