...
function RtspClient:processResponse(request, response)
	...
	--print('processResponse', statusCode, response.statusMessage)
...
function RtspClient:processAuthenticate(request, response)
	...
	if (self.rtspState == 0) or (self.rtspState == exports.STATE_INIT) then
		self:sendDESCRIBE()
	end
end
...
function RtspClient:sendSETUP()
	...
	elseif (self.mediaTracks == nil) or (#self.mediaTracks < 1) then
		self:emit('error', 'Invalid media tracks')
		self:close()
	end
	...
	if (not track) then
		self:emit('error', 'Invalid media track')
		self:close()
		return
	end
