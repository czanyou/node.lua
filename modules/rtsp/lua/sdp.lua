...
http://node.sz.com/?p=docs&f=vision_media_sdp.md
--]]
...
---@return string
function SdpMedia:getAttribute(name, parameterName)
    if (not self.attributes) then
        return
    end

    local attributeValue = self.attributes[name]
    if (not parameterName) then
        return attributeValue
    end

    local offset = 0
    local parameter = attributeValue

    -- parameters
    while (parameter) do
        _, offset, parameter = attributeValue:find("^([^;]+) ?", offset + 1)
        if (parameter) then
            offset = offset + 1

            local _, _, key, value = parameter:find("^([^=]+)=([^;]+)")
            key = string.trim(key)
            if (key == parameterName) then
                return value and string.trim(value)
            end
        end
    end
end
...
function SdpMedia:getFramerate(payload)
    local attribute = self:getAttribute('framerate')
    local value = tonumber(attribute)
...
function SdpMedia:getFramesize(payload)
    local attribute = self:getAttribute('framesize')
...
function SdpMedia:getRtpmap(payload)
    local attribute = self:getAttribute('rtpmap')
    if (not attribute) then
        return
    end
...
function SdpMedia:getFmtp(payload)
    local attribute = self:getAttribute('fmtp')
    if (not attribute) then
        return
    end
    ...
    -- value: payload
    _, offset, parameter = attribute:find("^([^ ]+) ?", offset)
    fmtp.payload = tonumber(parameter)

    -- parameters
    while (parameter) do
        _, offset, parameter = attribute:find("^([^;]+) ?", offset + 1)
        if (parameter) then
            offset = offset + 1
...
-- 返回指定的类型的 media
---@param type string 如 'video', 'audio', 等等
---@return SdpMedia
...
---@param media SdpMedia
---@param sdpLine string
local function decodeMedia(media, sdpLine)
    ...
    while true do
        ...
        -- console.log(index, offset, key, value)
