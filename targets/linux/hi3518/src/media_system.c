...
int MediaSystemInit(int mmzSize) 
{
	...
	memset(&bufferConfig, 0, sizeof(VB_CONF_S));
	...
	bufferConfig.astCommPool[0].u32BlkSize	= (UINT)(1280 * 720 * 2); // 720p
	bufferConfig.astCommPool[0].u32BlkCnt	= 6;

	bufferConfig.astCommPool[1].u32BlkSize	= (UINT)(640 * 360 * 2);	// 360p
	bufferConfig.astCommPool[1].u32BlkCnt	= 6;

	bufferConfig.astCommPool[2].u32BlkSize	= (UINT)(320 * 180 * 2);	// 180p
	bufferConfig.astCommPool[2].u32BlkCnt	= 6;

	bufferConfig.astCommPool[3].u32BlkSize	= (196 * 4);
	bufferConfig.astCommPool[3].u32BlkCnt	= 6;

	int ret = HI_MPI_VB_SetConf(&bufferConfig);
	if (ret != HI_SUCCESS) {
		LOG_E("HI_MPI_VB_SetConf failed with 0x%x\n", ret);
		return ret;
	}

	ret = HI_MPI_VB_Init();
	if (ret != HI_SUCCESS) {
		LOG_E("HI_MPI_VB_Init failed with 0x%x\n", ret);
		return ret;
	}
	...
	if (ret != HI_SUCCESS) {
		LOG_E("HI_MPI_SYS_SetConf failed with 0x%x!\n", ret);
		return ret;
	}

	ret = HI_MPI_SYS_Init();
	if (ret != HI_SUCCESS) {
		LOG_E("HI_MPI_SYS_Init failed with 0x%x!\n", ret);
		return ret;
	}
